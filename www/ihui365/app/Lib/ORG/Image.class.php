...
class Image {
     ...
     * @return string
     +----------------------------------------------------------
     */
     static function buildImageVerify( $length=4, $mode='1', $type='png', $width=300, $height=100,  $verifyName='verify') {
		 $size = 60;  
         $fontPath = "./static/ftxia/css/elephant.ttf";
     ...
     * @return string
     */
    static function buildImageVerifys($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        import('ORG.Util.String');
        $randval = String::randString($length, $mode);
        session($verifyName, md5($randval));
        $width = ($length * 10 + 10) > $width ? $length * 10 + 10 : $width;
