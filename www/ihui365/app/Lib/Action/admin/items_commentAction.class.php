...
class items_commentAction extends BackendAction
{
    ...
    public function index() {
        ...
        $list  = $this->_mod->field($prefix.'items_comment.add_time,'.$prefix.'items_comment.info,'.$prefix.'items_comment.uname,'.$prefix.'items.num_iid,'.$prefix.'items.title as item_name,'.$prefix.'items_comment.status,'.$prefix.'items_comment.id,'.$prefix.'items_comment.info')->join($prefix.'items ON '.$prefix.'items.id='.$prefix.'items_comment.item_id')->where($where)->order($sort . ' ' . $order)->limit($pager->firstRow.','.$pager->listRows)->select();
