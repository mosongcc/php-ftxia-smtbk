...
class robots_uzAction extends BackendAction {
	...
	public function collect(){
		...
		if($auto){
			...
			$np = $p+1;
			$result_data = $this->api_collect($date,$p,$yg);
	...
	public function api_collect($date,$p,$yg=0){
		M('robots_uz')->where(array('id'=>$date['id']))->save(array('last_page'=>$p,'last_time'=>time()));
		...
		$result						= $this->_get_list($map, $p,$yg);
		...
		foreach ($taobaoke_item_list as $key => $val) {
				if($map['shop_type'] == 'B'){
					if($map['shop_type'] == $val['shop_type']){
						...
						$val['recid'] = 0;
						$val['astime'] = date("Ymd",$val['coupon_start_time']);
						if(!$val['intro']){
							$val['intro'] = $val['title'];
						}								
     ...
     * 返回商品列表和总数
     */
    private function _get_list($map, $p , $yg=0) {
        $tb_top = $this->_get_tb_top();
