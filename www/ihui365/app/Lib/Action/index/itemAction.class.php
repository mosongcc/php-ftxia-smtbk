...
class itemAction extends FirstendAction {
	...
	public function _initialize() {
		...
		$tag_lists = M('tags')->where(array('status'=>'1'))->order('tcount desc')->limit('50')->select();
		$this->assign('tag_lists',$tag_lists);
		$class_array = array('','a1','a2','a3','a4','a5','a6');
		$this->assign('class_array',$class_array);


		$top = $this->_get_ftx_top();
	...
	public function _empty(){
		...
		$clicklength = strlen($item['click_url']);
		if($item['click_url']){
			if($clicklength<20){
				$item['click_url'] = '';
			}
		...
		$this->_config_seo(C('ftx_seo_config.item'), array(
			...
			if(C('ftx_desc_url')){
				...
				$desc=str_replace('href=','isconvert=1 rel="nofollow" href=',$desc);
			}else{
		...
		foreach($orlike as $key=>$val){
			...
			if($items[$key]['click_url']){
				$len = strlen($items[$key]['click_url']);
				if($len <20){
					$items[$key]['click_url'] = '';
				}
			}
