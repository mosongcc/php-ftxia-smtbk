...
.brand-banner img{vertical-align:top}
	...
	<div class="app">
		...
		<div class="fixtop">
			<span id="t-find"><a href="<?php echo U('brand/index');?>" class="btn btn-left btn-back"></a></span>
			<span id="t-index"><?php echo ($shop["nick"]); ?></span>
		</div>
		</header>
		<div class="brand-banner">
			<img src="<?php echo ($shop["enter_img"]); ?>">
		...
		<?php if(is_array($items)): $x = 0; $__LIST__ = $items;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$itemvol): $mod = ($x % 2 );++$x; if(is_array($itemvol)): $i = 0; $__LIST__ = $itemvol;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 4 );++$i;?><li>
				<a class="goods-aa"  href="<?php echo U('jump/index',array('iid'=>$item['num_iid']));?>" target="_blank">
					<img  src="__STATIC__/images/grey.gif" class="lazy" _src="<?php echo ($item["pic_url"]); ?>_310x310.jpg" />
				</a>
				<a  href="<?php echo U('jump/index',array('iid'=>$item['num_iid']));?>"  target="_blank"><h3><?php echo ($item["title"]); ?></h3>
				<div class="list-price buy "><span class="price-new"><i>￥</i><?php echo ($item["coupon_price"]); ?></span><i class="del">￥<?php echo ($item["price"]); ?></i><span class="good-btn"><?php if($item["shop_type"] == 'C' ): ?>淘宝<?php elseif($item["shop_type"] == 'B' ): ?>天猫<?php endif; ?></span></div></a>
