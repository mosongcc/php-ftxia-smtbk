...
<div class="jiu-nav-main">
	<div class="jiu-nav <?php echo C('ftx_site_width');?>">
		<div class="nav-item fl">
			<div class="item-list">
				...
				<?php if(is_array($cate_data)): $i = 0; $__LIST__ = $cate_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bcate): $mod = ($i % 2 );++$i; if(($bcate['pid'] == 0) AND ($cdi < 9)): $cdi++; ?>
					<li><a <?php if(($cid == $bcate['id']) OR ($cate_data[$cid]['pid'] == $bcate['id'])): ?>class="active"<?php endif; ?> href="<?php echo U('index/cate', array('cid'=>$bcate['id']));?>" title="<?php echo ($bcate["name"]); ?>"><?php echo ($bcate["name"]); ?></a></i><?php endif; endforeach; endif; else: echo "" ;endif; ?>
				</ul>
			</div>
		</div>
		...
		<div class="n_h">
			<span>排序：</span>
			<a href="<?php echo U('index/cate',array('cid'=>$cid,'sort'=>'default'));?>"  class=" <?php if($index_info['sort'] == 'default'): ?>active<?php endif; ?> ">默认</a>
			<a href="<?php echo U('index/cate',array('cid'=>$cid,'sort'=>'new'));?>" class=" <?php if($index_info['sort'] == 'new'): ?>active<?php endif; ?> ">最新</a>
			<a href="<?php echo U('index/cate',array('cid'=>$cid,'sort'=>'hot'));?>" class=" <?php if($index_info['sort'] == 'hot'): ?>active<?php endif; ?> ">最热</a></div>
		</div>
	</div>
</div>
 
<?php if(!empty($subnav)): ?><div class="jiu-nav-main jiu-nav-main-2" id="seclevel">
	<div class="jiu-nav <?php echo C('ftx_site_width');?>  seclevelinner">
		<div class="nav-item  ">
			<div class="item-list">
				<ul>
				<?php if(empty($cinfo['pid'])): ?><li><a href="<?php echo U('index/cate', array('cid'=>$cid));?>"   class="active">全部</a></li>
				<?php else: ?>
					<li><a href="<?php echo U('index/cate', array('cid'=>$cinfo['pid']));?>"   >全部</a></li><?php endif; ?>
				<?php if(is_array($subnav)): $i = 0; $__LIST__ = $subnav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bcate): $mod = ($i % 2 );++$i;?><li><a <?php if($cid == $bcate['id']): ?>class="active"<?php endif; ?> href="<?php echo U('index/cate', array('cid'=>$bcate['id']));?>" title="<?php echo ($bcate["name"]); ?>"><?php echo ($bcate["name"]); ?></a></i><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul>
			</div>
		</div>
	</div>
