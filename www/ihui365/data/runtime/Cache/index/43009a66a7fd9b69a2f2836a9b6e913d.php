...
$(function(){
	...
	$('.left-menu-ul li').each(function(i){
		...
		<div class="right">
			<div class="menu-tag">
				<ul>
					<li class="active"><a href="<?php echo U('user/mingxi');?>">收入明细
					<li><a href="<?php echo U('user/gift');?>">兑换明细</a></li>
				</ul>
			</div>
			<div class="u-tip">
				<p>当前积分：<em><?php echo ($score_total); ?></em> 分</p>
			</div>
			...
			<div class="log-table">
				<table border="0" cellspacing="0">
					...
					<?php if(!empty($logs_list)): if(is_array($logs_list)): $i = 0; $__LIST__ = $logs_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr <?php if(($mod) == "0"): ?>class="tr"<?php endif; ?> <?php if(($mod) == "1"): ?>class="tr-one"<?php endif; ?>>
						<td><?php echo L($val['action']);?></td>
						<td><?php if($val['score'] > 0): echo ($val["score"]); else: echo ($val["score"]); endif; ?></td>
