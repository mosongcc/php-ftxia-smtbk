...
<div class="jiu-nav-main">
	<div class="jiu-nav <?php echo C('ftx_site_width');?>">
		<div class="nav-item fl">
			<div class="item-list">
				<ul>
				<li ><a href="<?php echo U('brand/index');?>" <?php if(empty($cid)): ?>class="active"<?php endif; ?> >全部</a></li>
				<?php if(is_array($cats)): $i = 0; $__LIST__ = $cats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('brand/index',array('cid'=>$vol['cid'],'timeFilter'=>$timeFilter));?>"  <?php if($cid == $vol['cid']): ?>class="active"<?php endif; ?> ><?php echo ($vol["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul>
			</div>
		</div>
		<div class="n_h">
			<a href="<?php echo U('brand/index',array('cid'=>$cid));?>" <?php if(empty($timeFilter)): ?>class="active"<?php endif; ?>>默认</a>
			<a href="<?php echo U('brand/index',array('cid'=>$cid,'timeFilter'=>'today'));?>" <?php if($timeFilter == 'today'): ?>class="active"<?php endif; ?>>今日品牌</a>
			<a href="<?php echo U('brand/index',array('cid'=>$cid,'timeFilter'=>'last'));?>" <?php if($timeFilter == 'last'): ?>class="active"<?php endif; ?>>最后疯抢</a>
			<a href="<?php echo U('brand/index',array('cid'=>$cid,'timeFilter'=>'tomorrow'));?>" <?php if($timeFilter == 'tomorrow'): ?>class="active"<?php endif; ?>>明日预告</a>
			<a href="<?php echo U('brand/index',array('cid'=>$cid,'timeFilter'=>'old'));?>"      <?php if($timeFilter == 'old'): ?>class="active"<?php endif; ?>>往期活动</a>
		</div>

	</div>
</div>

<div class="bmain <?php echo C('ftx_site_width');?> pr mt25 clear">
        <div class="slide-logos clear">
		<span class="prev-btn" style="display: inline;"></span>
		<span class="next-btn" style="display: inline;"></span>
		<div class="logos-box">
			<ul>
			<li class="<?php echo C('ftx_site_width');?>">
			<?php if(is_array($brands)): $i = 0; $__LIST__ = $brands;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 24 );++$i;?><a href="<?php echo U('brand/dlist',array('uid'=>$val['uid']));?>" target="_blank">
					<img alt="<?php echo ($val["shop-name"]); ?>" src="<?php echo ($val["shop_icon"]); ?>">
					<div class="masking"></div>
					<span><?php echo ($val["discount"]); ?></span>
				</a>                                 
				<?php if(($mod) == "23"): ?></li><li class="<?php echo C('ftx_site_width');?>"><?php endif; endforeach; endif; else: echo "" ;endif; ?>
			</li>
			</ul>
		</div>
	</div>
...
<div class="bmain <?php echo C('ftx_site_width');?> m0a clear tag-list"">

<div class="brand-box new-brand">
<?php if(is_array($lists)): $ii = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($ii % 2 );++$ii;?><div class="bd">
	    <div class="brand-title clear">
                        <h3 class="name fl"><em class="  brand  fl"></em><a href="<?php echo U('brand/dlist',array('uid'=>$val['shop']['uid']));?>" target="_blank"><span> <?php echo ($val['shop']['nick']); ?> 品牌专场 </span></a></h3>
                        <div class="time fl">
                            <em class="icon clock fl"></em>

			    <?php if($val['shop']['ostime'] >= time()): ?><span class="fl left_time "><i class="i_time" style="display: none">1448416800</i>距活动开始<span class="endtime" data-time="<?php echo ($val['shop']['ostime']); ?>" data-id="s<?php echo ($ii); ?>" id="end_date_s<?php echo ($ii); ?>"> </span></span>
				<?php else: ?>
				<span class="fl left_time "><i class="i_time" style="display: none">1448416800</i>剩余<span class="endtime" data-time="<?php echo ($val['shop']['oetime']); ?>" data-id="s<?php echo ($ii); ?>" id="end_date_s<?php echo ($ii); ?>"> </span></span><?php endif; ?>


                            
                        </div>
                        <a href="<?php echo U('brand/dlist',array('uid'=>$val['shop']['uid']));?>" target="_blank" class="see-more fr"><em class="icon brand-other fl"></em><span>查看更多<?php echo ($val['shop']['items_total']); ?>款&gt;&gt;</span></a>
                    </div>
	...
	<ul class="goods-list <?php echo C('ftx_site_wc');?>">
		<?php if(is_array($val['items'])): $i = 0; $__LIST__ = array_slice($val['items'],0,12,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 4 );++$i;?><li  class="<?php if(($mod) == "3"): ?>last<?php endif; ?>">
			<div class="sid_<?php echo ($item["sellerId"]); ?>  list-good   <?php echo ($item["class"]); ?> <?php echo ($i); ?>" id="nid_<?php echo ($item["num_iid"]); ?>">
				<div class="good-pic">
...
<?php if(!empty($page)): ?><div class="main <?php echo C('ftx_site_width');?> clear">
	<div class="page">
