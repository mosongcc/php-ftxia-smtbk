...
<link rel="stylesheet" type="text/css" href="__STATIC__/8mobcom/css/article.css" />
...
<div class="jiu-nav-main">
	<div class="jiu-nav <?php echo C('ftx_site_width');?>">
		<div class="nav-item ">
			<div class="item-list">
				<ul>
				<li ><a href="<?php echo U('article/index');?>" <?php if($cid == 0): ?>class="active"<?php endif; ?> >全部</a></li>
				<?php if(is_array($acats)): $i = 0; $__LIST__ = $acats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('article/cate',array('id'=>$vol['id']));?>"  <?php if($cid == $vol['id']): ?>class="active"<?php endif; ?> ><?php echo ($vol["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul>
			</div>
		</div>
	</div>
</div>
<div class="main clear mb20 <?php echo C('ftx_site_width');?>">
	<div class="about <?php echo C('ftx_site_width');?> content">
		
		<div class="place-explain">
			<a target="_blank" href="<?php echo C('ftx_site_url');?>"><?php echo C('ftx_site_name');?></a>
			&nbsp;&gt;&nbsp;
				<a target="_blank" href="<?php echo U('article/index');?>">文章资讯</a>		
			&nbsp;&gt;&nbsp; 
			<?php if(!empty($all_cats[$article['cate_id']]['pid'])): ?><a target="_blank" href="<?php echo U('article/cate',array('id'=>$all_cats[$article['cate_id']]['pid']));?>" ><?php echo ($all_cats[$all_cats[$article['cate_id']]['pid']]['name']); ?></a>		
			&nbsp;&gt;&nbsp;<?php endif; ?>

			<a target="_blank" href="<?php echo U('article/cate',array('id'=>$all_cats[$article['cate_id']]['id']));?>" ><?php echo ($all_cats[$article['cate_id']]['name']); ?></a>
			&nbsp;&gt;&nbsp;
			<a class="bady-xx-seo" href="<?php echo U('article/read',array('id'=>$article['id']));?>"><?php echo ($article["title"]); ?></a>
		</div>
		 
		<div class="about_contain fl">
			<h2><?php echo ($article["title"]); ?></h2>
			<p class="gray tc mt10">时间：<?php echo (date("Y-m-d ",$article["add_time"])); ?>&nbsp;&nbsp;&nbsp;&nbsp;来源：<?php echo C('ftx_site_name');?>&nbsp;&nbsp;&nbsp;&nbsp;作者：<?php echo ($article["author"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;阅读：<?php echo ($article["hits"]); ?></p>
			<div class="about_content mt30">
				<?php echo ($article["info"]); ?>


				<div class="keyMode">
					<?php if(!empty($tag_list)): ?><div class="keyWord fl">关键词：
						<?php if(is_array($tag_list)): $i = 0; $__LIST__ = $tag_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tag): $mod = ($i % 2 );++$i;?><a href="<?php echo U('tag/'.$tag);?>" target="_blank"><?php echo ($tag); ?></a>&nbsp;&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
					</div><?php endif; ?>
					<!--百度分享 Star-->
					...
					<script>window._bd_share_config={"common":{"bdSnsKey":{},"bdText":"<?php echo ($article["title"]); ?>","bdMini":"2","bdMiniList":false,"bdPic":"","bdStyle":"1","bdSize":"24"},"share":{}};with(document)0[(getElementsByTagName('head')[0]||body).appendChild(createElement('script')).src='http://bdimg.share.baidu.com/static/api/js/share.js?v=89860593.js?cdnversion='+~(-new Date()/36e5)];</script>
					<!--百度分享 End-->
				</div>

				<div class="">
					<div class="fl">上一篇：<a href="<?php echo U('article/read',array('id'=>$pre_article['id']));?>"  ><?php echo ($pre_article["title"]); ?></a></div>
					<div class="fr">下一篇：<a href="<?php echo U('article/read',array('id'=>$next_article['id']));?>" ><?php echo ($next_article["title"]); ?></a></div>
				</div>
			</div>

			
			<div class="read_list">
				<p><a href="<?php echo U('article/index');?>" target="_blank">more+</a>大家都在看：</p>
				<ul>
				<?php $tag_article_class = new articleTag;$data = $tag_article_class->lists(array('type'=>'lists','num'=>'10','order'=>'hits desc','cache'=>'0','return'=>'data',)); if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('article/read',array('id'=>$val['id']));?>" target="_blank" title="<?php echo ($val["title"]); ?>"><?php echo ($val["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul>
			</div>
		...
		<div class="right_menu fr">
			...
			<div class="widget">
				<h3>热词标签</h3>
				<div class="tags">
					<?php if(is_array($tags)): $i = 0; $__LIST__ = $tags;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tag): $mod = ($i % 2 );++$i;?><a href="<?php echo U('tag/'.$tag['name']);?>" target="_blank" class="tag-link-20"  style="font-size: <?php echo rand(12,20);?>px;"><?php echo ($tag["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			...
			<div class="widget">
				<h3>文章推荐</h3>
				<ul>
				<?php if(is_array($new_article)): $i = 0; $__LIST__ = $new_article;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$art): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('article/read',array('id'=>$art['id']));?>" target="_blank" ><?php echo ($art["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul>
				<div class="clear"></div>
			</div>
			...
			<div class="widget art_item">
				...
				<?php $tag_item_class = new itemTag;$data = $tag_item_class->lists(array('type'=>'lists','num'=>'8','cache'=>'0','return'=>'data',)); if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="hotshopbox">
						<a href="<?php echo U('item/index',array('iid'=>$val['num_iid']));?>" target="_blank"  title="<?php echo ($val["title"]); ?>">
							<img class="home-thumb" src="<?php echo attach(get_thumb($val['pic_url'], '_b'),'item');?>" alt="<?php echo ($val["title"]); ?>" />
						</a>
