...
<!--main start -->
<div class="main <?php echo C('ftx_site_width');?> mb20 main-newgood">
		<div class="form-result">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php if(!empty($goods_list)): if(is_array($goods_list)): $i = 0; $__LIST__ = $goods_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
					<td><?php echo (date("Y-m-d H:i:s",$val["add_time"])); ?></td>
					<td style="text-align:left"><a href="http://item.taobao.com/item.htm?id=<?php echo ($val["num_iid"]); ?>" class="name" target="_blank"><?php echo ($val["title"]); ?></a></td>
					<td><?php echo ($val["price"]); ?></td>
					<td><?php echo ($val["coupon_price"]); ?></td>
					<td><?php if($val["ems"] == 1): ?>包邮<?php else: ?>不包邮<?php endif; ?></td>
					<td><?php echo ($val["volume"]); ?></td>
					<td><?php if(!empty($val['pay_id'])): if($val["pay_status"] == 1): ?><span>已付款</span>
					<?php else: ?>
					<span>未付款</span><?php endif; ?>
					<?php else: ?>
					免费报名<?php endif; ?></td>
					<td>
						<div class="red">
						<?php if($val["pass"] == 1): ?><span>通过</span>
						<br>
						<?php elseif($val["pass"] == 0): ?>
							<?php if($val["status"] == 'fail'): ?><span>未通过</span><br><span class="opt"><span><a href="<?php echo U('baoming/edit',array('id'=>$val['id']));?>">修改</a></span>&nbsp;</span>
							<?php else: ?>

								<?php if($val["pay_status"] == 1): ?><span>审核中</span>
								<?php else: ?>
								<span>待审核</span><?php endif; endif; endif; ?>
						</div>
					</td>
					<td class="last">
						<?php if(!empty($val['pay_id'])): if($val["pay_status"] == 1): ?>排期：<?php echo (date("m-d H:i",$val["coupon_start_time"])); ?><br />
							结束：<?php echo (date("m-d H:i",$val["coupon_end_time"])); ?>
							<?php else: ?>
								...
								<input name="optEmail" type="hidden" value="<?php echo C('ftx_alipay');?>"> 
								<input name="memo" type="hidden" value="订单号：<?php echo ($val["num_iid"]); ?>，金额<?php echo ($baoming_reason[$val['pay_id']][1]); ?>"> 
								<input id="payAmount" name="payAmount" type="hidden" value="<?php echo ($baoming_reason[$val['pay_id']][1]); ?>"> 
								<input id="title" name="title" type="hidden" value="<?php echo ($info["username"]); ?> 付费报名">
								<input type="image" class="btn-blue" src="__STATIC__/8mobcom/images/alipay.png" width="80px" border="0" name="submit" alt="付费报名">
								</form><?php endif; ?>

						<?php else: ?>
						<a href="<?php echo U('baoming/edit',array('id'=>$val['id']));?>" class="btn-blue">修改</a><?php endif; ?>
					</td>
					</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			</table>
					<?php else: ?>
			</table>
					<div class="tc f14 pt50">没有找到您要查询的信息哦，您可以先去<a href="<?php echo U('baoming/index');?>" style="color:#0289CD;">报名&gt;&gt;</a></div><?php endif; ?>
							...
							<div class="page"><div class="pageNav"> <?php echo ($page_bar); ?></div></div> 
		</div>
		    		</div>
	</div>
</div>
