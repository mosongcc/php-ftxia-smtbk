...
<div class="subnav">
	<div class="content_menu ib_a blue line_x">
    	<a class="add fb " href="<?php echo U('robots_uz/add_do');?>" ><em>添加采集器</em></a>
		<a class="add fb " href="javascript:auto_collect(0)" ><em>一键自动采集</em></a>
		<a class="add fb " href="javascript:auto_collect(1)" ><em>一键采集预告</em></a>
...
<div class="pad_lr_10" >
    ...
    <div class="J_tablelist table_list" data-acturi="<?php echo U('robots_uz/ajax_edit');?>">
    <table width="100%" cellspacing="0">
		...
		<td align="center"><img data-tdtype="toggle" data-id="<?php echo ($val["id"]); ?>" data-field="status" data-value="<?php echo ($val["status"]); ?>" src="__STATIC__/images/admin/toggle_<?php if($val["status"] == 0): ?>disabled<?php else: ?>enabled<?php endif; ?>.gif" /></td>
                <td align="center"><a href="javascript:collect(<?php echo ($val["id"]); ?>,1);">开始采集</a> |<a href="<?php echo u('robots_uz/edit', array('id'=>$val['id'], 'menuid'=>$menuid));?>"><?php echo L('edit');?></a> | <a href="javascript:void(0);" class="J_confirmurl" data-uri="<?php echo u('robots_uz/delete', array('id'=>$val['id']));?>" data-acttype="ajax" data-msg="<?php echo sprintf(L('confirm_delete_one'),$val['name']);?>"><?php echo L('delete');?></a></td>
...
<script>
    var collect_url = "<?php echo U('robots_uz/collect');?>";
    var id = 0;
	var p = 1;
    ...
    function auto_collect(yg){
        $.getJSON(collect_url, {yg:yg,auto:1}, function(result){
            if(result.status == 1){
                $.dialog({id:'cmt_ftxia', title:result.msg.title, content:result.data, padding:'', lock:true});
                rid = result.msg.rid;
                p = result.msg.np;
                
                setTimeout("auto_collect_page("+ rid +","+ p +","+ yg +")",1000);
            }else{
                $.ftxia.tip({content:result.msg});
            }
        });
    }
    function auto_collect_page(rid,p,yg){
        $.getJSON(collect_url, {rid:rid,p:p,yg:yg,auto:1}, function(result){
            if(result.status == 1){
